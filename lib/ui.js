...
function drawUI (store, tMan) {
  ...
  const listBoxes = tMan.lists.map((list, i) => {
    ...
    lBox.key(['h'], () => {
      ...
      tMan.activeListIdx = Math.max(0, tMan.activeListIdx - 1)
      listBoxes[tMan.activeListIdx].focus()

      const screenWidth = process.stdout.columns
      const nCols = Math.floor(screenWidth / MIN_COL_WIDTH)
      const offset = Math.max(tMan.activeListIdx - (nCols - 1), 0) * MIN_COL_WIDTH
    ...
    lBox.key(['l'], () => {
      const numLists = tMan.lists.length
      tMan.activeListIdx = Math.min(numLists - 1, tMan.activeListIdx + 1)
      listBoxes[tMan.activeListIdx].focus()
  ...
  cardForm.on('submit', (card) => {
    ...
    const list = tMan.lists[tMan.activeListIdx]
    let actionFn
    if (tMan.editingCard) {
      actionFn = 'editCard'
